<script lang="ts">
	...
	let breadcrumbs = $derived.by(() => {
		const crumbs: Array<{ title: string; url: string }> = [
			{ title: m.levels_root(), url: '/levels/list' }
		];
...
<Column gap="var(--space-6)">
	<Column gap="var(--space-2)">
		<Title>{currentNode?.title || m.levels_not_found()}</Title>
		<Subtitle>
			{#if currentNode}
				{#if currentNode.type === 'folder'}
					{#if currentNode.children.length === 0}
						{m.levels_folder_empty()}
					{:else}
						{getLevelCount(currentNode)} {getLevelCount(currentNode) !== 1 ? m.levels_puzzles() : m.levels_puzzle()} {m.levels_available()}
					{/if}
				{:else}
					{currentNode.description}
				{/if}
			{:else}
				{m.levels_path_not_found()}
			{/if}
		</Subtitle>
	</Column>
				...
				{#if child.type === 'folder'}
					...
					<Card>
						<Column gap="var(--space-3)">
							...
							<a class="btn btn--primary" href={getPathUrl([...pathSegments, child.id])}>{m.levels_browse()}</a>
						</Column>
					</Card>
				{:else}
						...
						<Column gap="var(--space-3)">
							<Row gap="var(--space-2)" style="align-items: center;">
								...
								{#if child.source === 'procgen'}
									<ToolTip text={m.levels_procgen_tooltip({ requiredCompletions: child.requiredCompletions })}>
								...
								{:else}
									<ToolTip text={m.levels_fixed_tooltip({ requiredCompletions: child.requiredCompletions })}>
										<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style="flex-shrink: 0;">
								...
								{/if}
								{#if validated}
									<ToolTip text={m.levels_validated_tooltip()}>
							...
							{#if bestScore}
								<div style="font-size: 14px; color: var(--color-muted);">
									{m.levels_best_score({ score: bestScore.score })}
								</div>
							{/if}
							<Row>
	...
	{:else if currentNode && currentNode.type === 'level'}
		<Column gap="var(--space-3)">
			<Subtitle>{@html m.levels_play_here({ levelId: currentNode.id })}</Subtitle>
			<a class="btn btn--primary" href="/levels/list">{m.levels_back_to_categories()}</a>
		</Column>
	{:else}
		<Column gap="var(--space-3)">
			<Subtitle>{m.levels_path_not_found()}</Subtitle>
			<a class="btn btn--primary" href="/levels/list">{m.levels_back_to_categories()}</a>
		</Column>
	{/if}
</Column>
